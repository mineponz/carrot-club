...
- **父（種牡馬）のチェックは「0件」の状態を作らない。** 最後の1つを外したら全部にチェックを
  戻す（`withAllWhenNoneSelected()`、2026-08-25）。全部外しても表は全頭に戻るので、チェックが
  空欄のままだと見た目と結果が食い違うため。全チェック＝絞っていない扱いなので、
  `isAllSelected()` で見て**条件に数えず・保存もせず・父の見出しは「すべて」**にする
  （全選択を保存すると次に開いたとき「1件の条件」と出て、絞り込みが勝手に開く）。
  `filterHorses()` 側の約束（空配列＝絞り込まない）は変えていない。性別のチェックは従来どおり
  「1つも選ばなければ全頭」のまま。
