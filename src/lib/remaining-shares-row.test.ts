...
test('remainingSharesCellHtml: 未発表（hasRemaining: null）は「発表待ち」', () => {
  const row: RemainingSharesRow = { id: '1', hasRemaining: null, shares: null, soldOutInRound: null };
  assert.equal(remainingSharesCellHtml(row), '発表待ち');
});

test('remainingSharesCellHtml: 対象外（満口）は「—」', () => {
  const row: RemainingSharesRow = { id: '1', hasRemaining: false, shares: null, soldOutInRound: null };
  assert.equal(remainingSharesCellHtml(row), '—');
});

test('remainingSharesCellHtml: 対象で口数未発表は「あり」バッジ', () => {
  const row: RemainingSharesRow = { id: '9', hasRemaining: true, shares: null, soldOutInRound: null };
  assert.equal(remainingSharesCellHtml(row), '<span class="remaining-badge">あり</span>');
});

test('remainingSharesCellHtml: 実数が出ている馬は「N口」バッジ', () => {
  const row: RemainingSharesRow = { id: '9', hasRemaining: true, shares: { kind: 'exact', count: 12 }, soldOutInRound: null };
...
test('remainingSharesCellHtml: 実数が出ない馬は「N口以上」バッジ', () => {
  const row: RemainingSharesRow = { id: '9', hasRemaining: true, shares: { kind: 'atLeast', count: 100 }, soldOutInRound: null };
