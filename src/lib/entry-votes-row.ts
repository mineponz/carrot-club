 ...
 * 初期HTMLと並べ替え後の表がずれる。
 *
 * 列は No・馬名・牡牝 ＋ 中間発表の各回。回数は固定ではないので、列定義は
 * `entryVoteColumns(snapshots)` でスナップショット配列から動的に組み立てる。
 * 狭い画面（SP）では No 列を隠し、馬名セルに `.sp-no` / `.sp-line` として畳む
 * （`tour-weight-row.ts` と同じ考え方）。
 */
 ...
 * 固定列（No・馬名・牡牝）のあとに、各回の全体票数列を `total:${i}` として並べる。
 * SP では No 列を隠すため、馬名・各回の見出しからは `spSortKey='id'` で募集番号順に並べる。
 */
...
export function entryVoteRowHtml(
  ...
  <td data-col="name" class="horse-name"><span class="sp-no">${id}.</span><a href="${href}">${name}</a><span class="sp-line">${sex}</span></td>
