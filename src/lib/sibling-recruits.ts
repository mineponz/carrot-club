 ...
 * `analysis/data/recruits.json`（2021〜2025年募集）に載っている馬だけ。
 *
 ...
 * 1. **母のnetkeiba個体ページID**が一致（＝同じ母の産駒）。確実だが、母のURLを持っているのは
 *    2024・2025年募集ぶんだけ（それ以前のスプレッドシートに母の列が無い）。
 * 2. **`Horse.sibling`（代表的な兄姉の実名）が過去募集馬の実名と一致**。1でカバーできない
 *    2021〜2023年募集ぶんを拾える。競走馬名は登録上ユニークなので同名の別馬は入らない。
 *
