 ...
 * 4. `/lottery/`      → `/2026/lottery/` … 抽選ステータス一覧ページ。tour-weight/votes と同じく
 *    中身が100%2026年募集馬なので年度付きが正本（本人承認 / 2026-09-05）。
 * 5. `/2026/`         → `/` … `/2026/` は**最新年度への別名**で、実体は `/` と同じもの。
 *    過去年度（`/2025/`）と同じ形で辿ってきた人を落とさないために受ける。
 *
 * ## 来年（2027年募集）の切替でやること
 * **この5本の向き先を2027に変えるだけ**（`/horses/`・`/tour-weight/`・`/votes/`・`/lottery/`・`/2026/`）。
...
/** 年度なしの抽選ステータス一覧ページ。tour-weight/votes と同じく恒久的に301で受け続ける。 */
const LEGACY_LOTTERY_PATH = '/lottery/';
...
export function redirectTarget(pathname: string): string | null {
  ...
  // 年度なしの抽選ステータス一覧ページ。`/2026/lottery/` は正本なので当然対象外。
  if (path === LEGACY_LOTTERY_PATH) return `${CURRENT_YEAR_PREFIX}lottery/`;
